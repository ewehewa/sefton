...
<div x-data="{ 
    withdrawMethod: '<?php echo e(request()->input("method") == "cryptocurrency" ? "Cryptocurrency" : ""); ?>',
    amount: '',
    accountName: '',
    accountNumber: '',
    bankName: '',
    ...
    validateAmount() {
        const maxBalance = <?php echo e(Auth::user()->account_bal); ?>;
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8" x-show="!withdrawMethod">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                ...
                <!-- Cryptocurrency -->
                <div id="crypto" @click="withdrawMethod = 'Cryptocurrency'; changeMethod()" class="cursor-pointer bg-white rounded-xl border border-gray-200 p-4 hover:border-primary-500 hover:shadow-md transition-all">
                    <div class="flex items-center mb-3">
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden" x-show="withdrawMethod && !showMoreMethods">
            ...
            <div class="p-6 md:p-8 pb-14">
                <form action="<?php echo e(route('internationaltransfer')); ?>" method="post" id="internationalTransferForm" @submit.prevent="previewTransfer()">
                    <?php echo csrf_field(); ?>
                        ...
                        <div class="mt-3 flex items-center justify-between">
                            ...
                            <div class="flex space-x-2">
                                <template x-if="balanceType === 'fiat'">
                                    <div class="flex space-x-2">
                                        <button type="button" @click="amount = '100'" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors"><?php echo e(Auth::user()->s_curr); ?>100</button>
                                        <button type="button" @click="amount = '500'" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors"><?php echo e(Auth::user()->s_curr); ?>500</button>
                                        <button type="button" @click="amount = '1000'" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors"><?php echo e(Auth::user()->s_curr); ?>1000</button>
                                        <button type="button" @click="amount = <?php echo e(Auth::user()->account_bal); ?>" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors">Max</button>
                                    </div>
                                </template>
                                <template x-if="balanceType === 'btc'">
                                    <div class="flex space-x-2">
                                        <button type="button" @click="amount = '0.001'" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors">0.001 BTC</button>
                                        <button type="button" @click="amount = '0.01'" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors">0.01 BTC</button>
                                        <button type="button" @click="amount = '0.1'" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors">0.1 BTC</button>
                                        <button type="button" @click="amount = <?php echo e(Auth::user()->btc_balance ?? 0); ?>" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md text-xs font-medium text-gray-700 transition-colors">Max</button>
                                    </div>
                                </template>
                            </div>
                        </div>
                ...
                <div x-show="amount > 0 && withdrawMethod !== ''" class="bg-gradient-to-br from-gray-50 to-white rounded-lg p-5 mb-6 border border-gray-100 shadow-sm">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <template x-if="balanceType === 'fiat'">
                                <span class="font-medium text-gray-700" x-text="amount ? '<?php echo e(Auth::user()->s_curr); ?>' + parseFloat(amount).toFixed(2) : '<?php echo e(Auth::user()->s_curr); ?>0.00'"></span>
                            </template>
                            <template x-if="balanceType === 'btc'">
                                <span class="font-medium text-gray-700" x-text="amount ? parseFloat(amount).toFixed(8) + ' BTC' : '0.00000000 BTC'"></span>
                            </template>
                        </div>
                        ...
                        <div class="border-t border-gray-200 pt-3 mt-3">
                            <div class="flex justify-between items-center">
                                ...
                                <template x-if="balanceType === 'fiat'">
                                    <span class="font-bold text-xl text-gray-900" x-text="amount ? '<?php echo e(Auth::user()->s_curr); ?>' + parseFloat(amount).toFixed(2) : '<?php echo e(Auth::user()->s_curr); ?>0.00'"></span>
                                </template>
                                <template x-if="balanceType === 'btc'">
                                    <span class="font-bold text-xl text-gray-900" x-text="amount ? parseFloat(amount).toFixed(8) + ' BTC' : '0.00000000 BTC'"></span>
                                </template>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center pt-1">
                            ...
                            <template x-if="balanceType === 'fiat'">
                                <span class="text-gray-900" x-text="'<?php echo e(Auth::user()->s_curr); ?>' + (<?php echo e(Auth::user()->account_bal); ?> - (parseFloat(amount) || 0)).toFixed(2)"></span>
                            </template>
                            <template x-if="balanceType === 'btc'">
                                <span class="text-gray-900" x-text="(<?php echo e(Auth::user()->btc_balance ?? 0); ?> - (parseFloat(amount) || 0)).toFixed(8) + ' BTC'"></span>
                            </template>
                        </div>
                        ...
                        <div class="flex justify-between items-center pt-1" x-show="amount && parseFloat(amount) > 0">
                            <span class="font-medium text-gray-500">Equivalent Value</span>
                            <template x-if="balanceType === 'fiat'">
                                <span class="text-gray-700" x-text="'≈ ' + (parseFloat(amount) / <?php echo e($btc_rate); ?>).toLocaleString('en-US', {minimumFractionDigits: 8, maximumFractionDigits: 8}) + ' BTC'"></span>
                            </template>
                            <template x-if="balanceType === 'btc'">
                                <span class="text-gray-700" x-text="'≈ <?php echo e(Auth::user()->s_curr); ?>' + (parseFloat(amount) * <?php echo e($btc_rate); ?>).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                            </template>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-5 border-t border-gray-200 pt-4">
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between text-sm">
                                ...
                                <template x-if="balanceType === 'fiat'">
                                    <span class="text-gray-900 font-bold" x-text="'<?php echo e(Auth::user()->s_curr); ?>' + parseFloat(amount).toFixed(2)"></span>
                                </template>
                                <template x-if="balanceType === 'btc'">
                                    <span class="text-gray-900 font-bold" x-text="parseFloat(amount).toFixed(8) + ' BTC'"></span>
                                </template>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="font-medium text-gray-500">Source</span>
                                <template x-if="balanceType === 'fiat'">
                                    <span class="text-gray-900">Fiat Balance</span>
                                </template>
                                <template x-if="balanceType === 'btc'">
                                    <span class="text-gray-900">Bitcoin Balance</span>
                                </template>
                            </div>
                            ...
                            <div class="pt-3 border-t border-gray-200">
                                ...
                                <div class="flex justify-between text-sm mt-1">
                                    ...
                                    <template x-if="balanceType === 'fiat'">
                                        <span class="text-gray-900" x-text="'<?php echo e(Auth::user()->s_curr); ?>' + (<?php echo e(Auth::user()->account_bal); ?> - parseFloat(amount)).toFixed(2)"></span>
                                    </template>
                                    <template x-if="balanceType === 'btc'">
                                        <span class="text-gray-900" x-text="(<?php echo e(Auth::user()->btc_balance ?? 0); ?> - parseFloat(amount)).toFixed(8) + ' BTC'"></span>
                                    </template>
                                </div>
                                <div class="flex justify-between text-sm mt-1" x-show="amount && parseFloat(amount) > 0">
                                    <span class="font-medium text-gray-500">Equivalent Value</span>
                                    <template x-if="balanceType === 'fiat'">
                                        <span class="text-gray-900" x-text="'≈ ' + (parseFloat(amount) / <?php echo e($btc_rate); ?>).toLocaleString('en-US', {minimumFractionDigits: 8, maximumFractionDigits: 8}) + ' BTC'"></span>
                                    </template>
                                    <template x-if="balanceType === 'btc'">
                                        <span class="text-gray-900" x-text="'≈ <?php echo e(Auth::user()->s_curr); ?>' + (parseFloat(amount) * <?php echo e($btc_rate); ?>).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                                    </template>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4 text-left border border-gray-200 mb-4">
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                            ...
                            <p class="font-medium text-gray-900" id="modalMethod"></p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-medium text-gray-900" id="modalCountry"></p>
                        </div>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Scroll to the form section if cryptocurrency was selected
        if (document.querySelector('[x-data]').__x.$data.withdrawMethod === 'Cryptocurrency') {
            // Scroll to the form section
            setTimeout(function() {
                window.scrollTo({
                    top: document.querySelector('.max-w-4xl.mx-auto').offsetTop - 50,
                    behavior: 'smooth'
                });
            }, 300);
        }
...
<?php echo $__env->make('layouts.dash2', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/atlantictruist/public_html/resources/views/user/international.blade.php ENDPATH**/ ?>